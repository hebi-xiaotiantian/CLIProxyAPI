...
也可以按账号在 auth 文件（`auths/*.json`）中设置字段覆盖全局开关，
HTTP/SSE 与 WebSocket 请求都会生效，响应中的上游标识会自动还原为客户端原值：
...
- 模式含义：
  ...
  - `full`：installation + session + thread 全部收敛为账号级恒定值
    （1 台设备 + 1 会话 + 1 线程，最激进）；
  - `off`：关闭全部身份混淆。
  - 全局开关与账号字段都不设置时，保持 `codex.identity-confuse` 的旧行为。
