...
var activePlugin = newAccountPoolPlugin(cHostCaller{})
...
func cliproxyPluginCall(method *C.char, request *C.uint8_t, requestLen C.size_t, response *C.cliproxy_buffer) C.int {
	...
	raw := safeCall(func() ([]byte, error) {
		if method == nil {
			return errorEnvelope("invalid_method", "method is required", false, 0), nil
		}
...
func handleMethod(method string, request []byte) ([]byte, error) {
	...
	case pluginabi.MethodPluginRegister, pluginabi.MethodPluginReconfigure:
		if errConfigure := activePlugin.configure(request); errConfigure != nil {
			return nil, errConfigure
		}
		return okEnvelope(pluginRegistration())
	case pluginabi.MethodSchedulerPick:
		return activePlugin.pick(request)
	case pluginabi.MethodUsageHandle:
		return activePlugin.handleUsage(request)
	case pluginabi.MethodManagementRegister:
		return okEnvelope(managementRegistration())
	case pluginabi.MethodManagementHandle:
		return activePlugin.handleManagement(request)
	default:
		return errorEnvelope("unknown_method", "unknown method: "+method, false, 0), nil
	}
}

func safeCall(call func() ([]byte, error)) (raw []byte) {
	defer func() {
		if recovered := recover(); recovered != nil {
			raw = errorEnvelope("plugin_panic", fmt.Sprintf("plugin panic: %v", recovered), false, 500)
		}
	}()
	result, errCall := call()
	if errCall != nil {
		return errorEnvelope("plugin_error", sanitizeText(errCall.Error()), false, 500)
	}
	return result
}
...
func (cHostCaller) Call(method string, payload any) (json.RawMessage, error) {
